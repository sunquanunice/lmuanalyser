...
public class DotWriter extends AbstractWriter {
	...
	public byte[] writeModel(Model model) throws WriterException {
		...
		Collection<Entity> visibleEntities = (Collection<Entity>) ModelElement
				.findVisibleModelElements(model.getEntities());

		for (Entity entity : visibleEntities) {
			Collection<Attribute> visibleAttributes = (List<Attribute>) ModelElement
					.findVisibleModelElements(entity.getAttributes());
			Collection<Operation> visibleOperations = (List<Operation>) ModelElement
					.findVisibleModelElements(entity.getOperations());
			...
			if (entity instanceof ProjectEntity) {
				buf.append("shape=\"ellipse\", fillcolor=\"green:darkorchid\", style=filled");
				isRecord = false;
			} else if(entity instanceof PackageEntity) {
				buf.append("shape=\"tab\", fillcolor=\"bisque:brown\", style=filled");
				isRecord = false;
			} else if(entity instanceof JarEntity) {
				buf.append("shape=\"component\", fillcolor=\"antiquewhite:aquamarine\", style=filled");
				isRecord = false;
			} else {
		...
		for (Relation relation : model.getRelations()) {
			...
			if (relation.getTailEntity().isVisible() && relation.getHeadEntity().isVisible()) {
				if (relation instanceof DependenceRelation) {
					...
					buf.append("\n\t");
					buf.append(
							quoteNodeNameIfNecessary(String.valueOf(assoc.getContainedEntity().getName().hashCode())));
					buf.append(" -> ");
					buf.append(
							quoteNodeNameIfNecessary(String.valueOf(assoc.getContainerEntity().getName().hashCode())));
					...
					buf.append(" -> ");
					buf.append(
							quoteNodeNameIfNecessary(String.valueOf(heritage.getSuperEntity().getName().hashCode())));
